...
xMap<K, V>::xMap(const xMap<K, V>& map) {
  // TODO YOUR CODE IS HERE
  //!   this->deleteKeys = nullptr;  this->deleteValues = nullptr;
    ...
    this->deleteKeys = nullptr;   // Copy the function pointers
    this->deleteValues = nullptr; // Copy the function pointers
    
    // Perform deep copy of entries
    for (int idx = 0; idx < map.capacity; idx++) {
        DLinkedList<Entry*>& list = map.table[idx];
        for (auto pEntry : list) {
            // Create new pointers for keys and values if they are pointers
            K newKey = pEntry->key;
            V newValue = pEntry->value;
            if constexpr (std::is_pointer<K>::value) {
                newKey = new typename std::remove_pointer<K>::type(*pEntry->key);
            }
            if constexpr (std::is_pointer<V>::value) {
                newValue = new typename std::remove_pointer<V>::type(*pEntry->value);
            }
...
xMap<K, V>& xMap<K, V>::operator=(const xMap<K, V>& map) {
  ...
  if (this != &map) {
    ...
    this->valueEqual = map.valueEqual;
    this->keyEqual = map.keyEqual;
    this->deleteKeys = map.deleteKeys;   // Copy the function pointers
    this->deleteValues = map.deleteValues; // Copy the function pointers
    // Perform deep copy of entries
    for (int idx = 0; idx < map.capacity; idx++) {
        DLinkedList<Entry*>& list = map.table[idx];
        for (auto pEntry : list) {
...
V xMap<K, V>::put(K key, V value) {
 int index = this->hashCode(key, capacity);
   V retValue = value; // retValue là giá trị mới được khởi tạo ban đầu
   typename DLinkedList<Entry*>::Iterator it = table[index].begin();
   while (it != table[index].end()) {
       Entry* entry = *it;
       if (this->keyEQ(entry->key, key)) {
           retValue = entry->value; // backup giá trị cũ
           entry->value = value;    // cập nhật thành giá trị mới
           return retValue;
       }
       ++it;
   }
   // Nếu không tìm thấy key, thêm một entry mới vào cuối danh sách tại bucket
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K)) {
    ...
    while (it != table[index].end()) {
        ...
        if (this->keyEQ(entry->key, key)) {
            ...
            table[index].removeItem(entry, &xMap<K,V>::deleteEntry);  // 👈 dùng hàm chuẩn
            count--;
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)) {
    ...
    return false; // không tìm thấy key-value pair
}
template <class K, class V>
...
DLinkedList<int> xMap<K, V>::clashes() {
  // TODO YOUR CODE IS HERE
  DLinkedList<int> clashIndices;
  for (int i = 0; i < capacity; i++) {
         int count = table[i].size(); 
         clashIndices.add(count);
  }
  return clashIndices;
}
